...
parser.add_argument('--eval_mode', type=int,
                    help=' 1 for Yang setup or 2 for Cybulska setup (recommended)')
...
def main():
    ...
    if args.eval_mode == 1:  # Yang setup
        logger.info('Creating event mentions key file')
        out_file = os.path.join(args.gold_files_dir, 'CD_test_event_clusters.key_conll')
        write_span_based_cd_coref_clusters(test_data, out_file, is_event=True, is_gold=True,
                                           use_gold_mentions=True)
        logger.info('Creating entity mentions key file')
        out_file = os.path.join(args.gold_files_dir, 'CD_test_entity_clusters.key_conll')
